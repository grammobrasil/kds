...
# Function to get all values from distinct nutrients columns in 'pedidosbox' and create a new table with one merges column 'produto', based on 'nutrientes' passed - this is necessary because 'pedidobox' have a column for each product, 
def function_lista_pedidos_por_prod(pedidos, nutr):
    ...
    for value in nutr:
      # rename the columns
      pedidos_nutr = pedidos.rename(columns={value : 'produto', str(value+' Gr'): 'g'})
      # Convert 'g' string to 0
      pedidos_nutr = pedidos_nutr[pd.to_numeric(pedidos_nutr['g'], errors='coerce').notnull()]
      # convert 'g' to float
    ...
    # the final table with all 'pedidos' and a collumn 'produto' with the id for each one
...
def produtos_from_detalhes(detalhes):
    ...
    for pedidos_value in detalhes["pedidos"]:
        ...
        try: pedidos_value["itens"]
        except: pass
        else:
            # iterate over each 'item' inside a 'pedido'
            for items_value in pedidos_value["itens"]:
                list_produtos = []
                # convert the '_id' of each 'produto' in the data from 'produto' collection
                for conteudo_value in items_value["conteúdo"]:
                    produto = client.grammo.produtos.find_one({"_id": ObjectId(conteudo_value['produto'])})
                    list_produtos.append(produto["dados"]["nome"])
                    
    return detalhes

# FUNCTION TO RETURN A DICT WITH THE 'produtos' DETAILS BASED ON THE 'compra produto _id'
def produtos_from_compras(id_compra):
    ...
    try:
        ...
        for pedido in compra_produtos:
            ...
            pedido_dict[pedido['_id']]['entrega_hora'] = conteúdo['entrega_hora']
