...
from kds.functions_bubble import *
from config import Config

import xmltodict, json, datetime, re
...
def NF_grammo(dados_compra, dados_cliente=False):
    ...
    emitente = Emitente(
        ...
        inscricao_estadual='0963861174', # numero de IE da empresa
    ...
    nota_fiscal = NotaFiscal(
       ...
       serie='1',
       numero_nf = dados_compra['nf_num'],           # Número do Documento Fiscal.
       ...
       informacoes_adicionais_interesse_fisco='DOCUMENTO EMITIDO POR ME OU EPP OPTANTE PELO SIMPLES NACIONAL NÃO GERA DIREITO A CRÉDITO FISCAL DE IPI',
    ...
    if dados_cliente:
        cliente = Cliente(
            ...
            numero_documento = dados_cliente['CPF'], # numero do cpf ou cnpj
    ...
    nota_fiscal.adicionar_produto_servico(
        ...
        valor_unitario_comercial=Decimal(str(dados_compra['valor_total'])),  # preço unitário
        valor_total_bruto=Decimal(str(dados_compra['valor_total'])),       # preço total
...
def NF_send(compra_num, cliente=False):
    ...
    last_nf_num = list(client.grammo.nf.aggregate([{"$sort": {"_id": -1}}]))[0]['_id']
    ...
    dados_compra['valor_total'] = round(resumo_compra['bubble']['valor total'], 2)

    # Grab details of "Compra" (User info, address, etc) if True
    dados_cliente = {}
    if cliente:
        detalhe_compra = compra_lookup(resumo_compra['bubble']['_id'])
        end_dict = NF_end_split(detalhe_compra[0]['conteúdo_pedido'][0]['end'][0]['endereços']['geo'])
        dados_cliente['nome'] = detalhe_compra[0]['conteúdo_pedido'][0]['usr'][0]['bio']['nome']
        dados_cliente['CPF'] = detalhe_compra[0]['conteúdo_pedido'][0]['usr'][0]['bio']['CPF']
        dados_cliente['email'] = detalhe_compra[0]['conteúdo_pedido'][0]['usr'][0]['contato']['email']
        dados_cliente['telefone'] = detalhe_compra[0]['conteúdo_pedido'][0]['usr'][0]['contato']['fone']
